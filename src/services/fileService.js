...
export async function uploadCV(token, jobSeekerId, file) {
  ...
  try {
    const response = await fetch(
      `${API_URL}/JobSeeker/${jobSeekerId}/upload-cv`,
      {
        method: "POST",
        headers: {
...
export async function getCVInfo(token, jobSeekerId) {
  ...
  try {
    const response = await fetch(
      `${API_URL}/JobSeeker/${jobSeekerId}/cv-info`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
...
export async function downloadCV(token, fileId) {
  ...
  try {
    console.log(
      `Attempting to download CV from: ${API_URL}/FileUpload/cv/${fileId}`
    );
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to download CV: ${response.status} - ${errorText}`
      );
    }
...
export async function uploadProfilePicture(token, file) {
  ...
  try {
    const response = await fetch(
      `${API_URL}/FileUpload/upload-profile-picture`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
...
export async function uploadProfilePictureBase64(
  token,
  base64String,
  ...
  try {
    const response = await fetch(
      `${API_URL}/FileUpload/upload-profile-picture-base64`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
...
export async function deleteProfilePicture(token, fileId) {
  ...
  try {
    const response = await fetch(
      `${API_URL}/FileUpload/profile-picture/${fileId}`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `Failed to delete profile picture: ${response.status} - ${errorText}`
      );
    }
